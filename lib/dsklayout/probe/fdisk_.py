...
class _Parser:
    ...
    def _parse_paragraph(content, paragraph):
        ...
        for line in paragraph.splitlines():
            _Parser._parse_line(disk, line)
        if 'table-header' in disk and 'table-rows' in disk:
            _Parser._postprocess_table(disk)
    ...
    def _parse_line(disk, line):
        for expr in _Parser._expressions:
            match = re.match(expr, line)
            if match:
                _Parser._store_parsed_line(disk, match)
                return True
    ...
    def _store_parsed_line(disk, match):
        ...
        if 'header' in groups:
            _Parser._store_table_header(disk, groups)
        elif 'row' in groups:
            _Parser._store_table_row(disk, groups)
        elif 'empty' not in groups:
            _Parser._store_disk_info(disk, groups)
    ...
    def _store_disk_info(disk, groups):
        disk.update({k: _Parser._convert(k, v) for k, v in groups.items()})

    @staticmethod
    def _convert(key, val):
        conv = _Parser._converters.get(key, lambda x: x)
        return conv(val)
    ...
    def _postprocess_table(disk):
        ...
        pattern = _Parser._build_row_pattern(header, rows)
        ranges = _Parser._determine_columns(header, pattern)
        disk['partitions'] = []
        keys = [k.lower() for k in header.split()]
        disk['columns'] = header.split()
        for row in rows:
            entry = {k: _Parser._convert(k, row[slice(*ranges[k])].strip())
                     for k in keys}
    ...
    def _build_row_pattern(header, rows):
        ...
        pattern = maxlen * [' ']
        for line in lines:
            _Parser._update_row_pattern(pattern, line)
        return ''.join(pattern)
    ...
    def _determine_columns(header, pattern):
        rng = dict()
        cols = header.split()
        nongreedy = [c for c in cols if c.lower() not in _Parser._greedy]
        greedy = [c for c in cols if c.lower() in _Parser._greedy]
        for col in nongreedy:
            key = col.lower()
            rng[key] = _Parser._determine_column(header, col, pattern)
        for col in greedy:
            key = col.lower()
            rng[key] = _Parser._determine_column(header, col, pattern, rng)
        return rng
    ...
    def _determine_column(header, col, pattern, rng=None):
        ...
        right = left + len(col)
        key = col.lower()
        align = _Parser._alignment.get(key, '?')
        if align == 'l':
            right = _Parser._adjust_right(pattern, left, right, rng)
        elif align == 'r':
            left = _Parser._adjust_left(pattern, left, right, rng)
        return (left, right)
...
class FdiskProbe(backtick_.BackTickProbe):
    ...
    def partab(self, name):
        ...
        return misc_.rekey_pt(ent, self._pt_map, self._pt_p_map, ('columns',))
